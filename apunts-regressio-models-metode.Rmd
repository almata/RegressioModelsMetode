...
* Contínues (Regressió)
* Categòriques (Anàlisi de la variància)
...
_Simple: one predictor variable. Multiple: two or more predictor variables._
...
* $b_1 > 0$ as _x_ increases, _y_ tends to increase.
* $b_1 < 0$ as _x_ increases, _y_ tends to decrease.
...
To get an idea of how precise future predictions would be, we need to know how much the responses (_y_) vary around the (unknown) mean population regression line $\mu_Y = E(Y) = \beta_0 + \beta_1x$. As stated earlier, $\sigma^2$ quantifies this variance in the responses. The smaller this $\sigma^2$ value, the more accurate will be our predictions. But we will rarely know its true value as it is a population parameter. The best we can do is estimate it.
...
* _n - 1_ because we would really like to use the unknown population mean $\mu$, but we don't know its value and so we estimate it with $\bar{y}$, losing one degree of freedom.
...
* _n - 2_ because in using $\hat{y_i}$ to estimate $\mu_Y$, we effectively estimate two parameters: the population intercept $\beta_0$ and the population slope $\beta_1$. That is, we lose two degrees of freedom.
...
2. A large $r^2$ value should not be interpreted as meaning that the estimated regression line fits the data well. Another function (i.e. a curve) might better describe the trend in the data. So, it is important to plot the data and check by ourselves.
...
* If the _P_-value is larger than the significance level $\alpha$, we fail to reject the null hypothesis. We conclude "there is not enough evidence at the $\alpha$ level to conclude that there is a linear relationship in the population between the predictor _x_ and response _y_".
...
Con el método de los mínimos cuadrados podemos obtener un modelo de regresión simple o con una sola variable independiente:

$$y_i = \beta_0 + \beta_1x_i + \epsilon_i $$

O un modelo de regresión múltiple o con más de una variable independiente:
...
* Si el modelo teórico no es lineal, se puede, en muchos casos, transformar en lineal p.ej. mediante la aplicación de logaritmos.
* Si no es lineal, se puede estudiar un modelo de regresión polinómico.
